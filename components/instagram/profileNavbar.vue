<template>
  <div
    class="container"
    :style="{
      backgroundColor: $store.state.colorMode.background,
      color: $store.state.colorMode.color,
    }"
  >
    <div class="null hidden fg-1">logo</div>
    <div class="searchbarContainer">
      <div class="searchbar">
        <span class="fas fa-search"></span>
        <input
          type="text"
          v-model="searchUser"
          ref="searchInput"
          @click="visible = true"
        />
      </div>
      <div class="dropdown" v-if="users.length > 0 && visible">
        <ul class="cardContainer">
          <li
            class="card"
            v-for="user in users"
            :key="user.pk"
            @click="$router.push(user.username)"
          >
            <div class="image">
              <img :src="user.profile_pic_url" />
            </div>
            <div class="texts">
              <p>{{ user.username }}</p>
              <p style="color: #8e8ea7">{{ user.full_name }}</p>
            </div>
          </li>
        </ul>
      </div>
    </div>

    <nav
      class="navbar fg"
      :style="{
        backgroundColor: $store.state.colorMode.background,
        color: $store.state.colorMode.color,
      }"
    >
      <nuxt-link
        to="/stalkers"
        tag="div"
        class="icon"
        :style="{
          backgroundColor: $store.state.colorMode.background,
          color: $store.state.colorMode.color,
        }"
      >
        <img src="@/assets/navbarSvgFiles/hacker-black.svg" />
      </nuxt-link>
      <nuxt-link to="/gtbot" tag="div" class="icon">
        <img src="@/assets/navbarSvgFiles/robot-black.svg" />
      </nuxt-link>
      <nuxt-link to="/store" tag="div" class="icon">
        <img src="@/assets/navbarSvgFiles/shop-black.svg" />
      </nuxt-link>
      <nuxt-link to="/settings" tag="div" class="icon">
        <img src="@/assets/navbarSvgFiles/settings-black.svg" />
      </nuxt-link>
      <nuxt-link to="/home" tag="div" class="icon">
        <img
          :src="$store.getters.getProfilePhoto"
          class="pp"
          alt="profile-photo"
        />
      </nuxt-link>
    </nav>
  </div>
</template>

<script>
export default {
  name: "Profile-Navbar",
  data() {
    return {
      searchUser: "",
      typing: false,
      users: [],
      visible: true,
    };
  },
  created() {
    document.addEventListener("click", this.documentClick);
  },
  destroyed() {
    // important to clean up!!
    document.removeEventListener("click", this.documentClick);
  },
  methods: {
    documentClick(e) {
      const input = this.$refs.searchInput;
      if (e.target !== input) return (this.visible = false);
    },

    async search() {
      const { users } = await this.$axios.$post("/" + this.searchUser);
      this.users = users;
    },
  },

  watch: {
    searchUser() {
      if (this.searchUser.trim() == "") return (this.visible = false);
      if (!this.typing) {
        setTimeout(() => {
          this.search();
          this.typing = false;
        }, 500);
      }
      this.typing = true;
      this.visible = true;
    },
  },
};
</script>

<style lang="scss" scoped>
.container {
  width: 935px;
  height: 54px;
  display: flex;
  align-items: center;
  border-bottom: 1px solid #dbdbdb;
  justify-content: space-between;

  .searchbarContainer {
    width: 375px;
    display: flex;
    justify-content: flex-end;
    position: relative;

    .searchbar {
      width: 215px;
      background-color: #fafafa;
      padding: 5px 10px;
      color: #cbccce;
      outline: 0;
      border: 1px solid rgba(var(--b6a, 219, 219, 219), 1);
      border-radius: 3px;
      display: flex;
      align-items: center;

      span {
        font-size: 13px;
      }

      input {
        background-color: transparent;
        border: 0px;
        outline: 0px;
        padding-left: 5px;
      }
    }
  }

  .dropdown {
    position: absolute;
    width: 375px;
    height: 362px;
    left: 20%;
    top: 40px;
    background-color: #ffffff;
    box-shadow: 0 0 5px 1px rgba(var(--jb7, 0, 0, 0), 0.0975);
    overflow-y: auto;
    cursor: default;

    .cardContainer {
      margin-top: 10px;

      .card {
        cursor: pointer;
        width: 100%;
        height: 54px;
        padding: 16px 8px;
        color: #292929;
        font-size: 14px;
        display: flex;
        align-items: center;
        font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto,
          Oxygen, Ubuntu, Cantarell, "Open Sans", "Helvetica Neue", sans-serif;

        .image {
          margin-right: 0.9em;

          img {
            width: 44px;
            height: 44px;
            border-radius: 50%;
          }
        }

        .texts {
          height: 44px;
          display: flex;
          flex-direction: column;
          justify-content: space-around;
        }

        &:hover {
          background-color: #fafafa;
        }
      }
    }
  }

  .navbar {
    display: flex;
    justify-content: flex-end;

    .icon {
      cursor: pointer;
      margin-left: 22px;

      img {
        width: 22px;
        height: 22px;
      }

      .pp {
        border-radius: 60px;
      }
    }
  }

  .fg {
    width: 360px;
  }
}
</style>
