...
export default {
  data() {
    return {
      username: "erdemefe07",
      password: "62156160ASD.12362156160",
      isActive: false,
      visible: false,
      type: "password",
    };
  ...
  methods: {
    async login() {
      const { session } = await this.$axios
        .$post("/login", {
          username: this.username,
          password: this.password,
        })
        .catch(this.loginErrorHandle);

      this.$store.commit("setSelfSession", session);
    ...
    // get response.data as err -- more detail at: https://medium.com/@pyrolistical/destructuring-nested-objects-9dabdd01a3b8
    loginErrorHandle({ response: { data: err } }) {
      switch (err.name) {
        case "IgResponseError":
          if (err.message == "Please wait a few minutes before you try again")
        ...
        case "IgLoginInvalidUserError":
          alert(
            "Kullanıcı adı bir hesaba ait değil. Lütfen kullanıcı adınızı kontrol edip tekrar deneyin."
          );
          break;
